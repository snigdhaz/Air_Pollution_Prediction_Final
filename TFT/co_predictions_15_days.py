...
checkpoint_callback = ModelCheckpoint(
    dirpath="checkpoints-co-15-days",
...
for start_idx in range(0, total_steps, step_size):
    ...
    actual_chunk = true_chunk["co"].values                   # scaled [0,1]
...
print(f"CO  RMSE: {rmse:.4f} µg/m³  |  MAE: {mae:.4f} µg/m³")
...
plt.plot(all_actuals, label="Actual CO", color="blue")
plt.plot(all_preds, label="Predicted CO", color="red", alpha=0.7)
plt.xlabel("Time Index")
plt.ylabel("CO")
plt.title("CO Predictions vs Actuals - 15 days window")
plt.legend()
plt.savefig("co_predictions_15_days.png", dpi=300, bbox_inches="tight")
...
def plot_loss_history(loss_history):
    ...
    plt.tight_layout()
    plt.savefig("loss_curve_co_15_days.png", dpi=300)
