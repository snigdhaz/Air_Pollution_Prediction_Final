...
checkpoint_callback = ModelCheckpoint(
    dirpath="checkpoints-no2-15-days",
...
target_idx = feature_cols.index("no2")
...
for start_idx in range(0, total_steps, step_size):
    ...
    actual_chunk = true_chunk["no2"].values                   # scaled [0,1]
...
print(f"NO2  RMSE: {rmse:.4f} µg/m³  |  MAE: {mae:.4f} µg/m³")
...
plt.plot(all_actuals, label="Actual NO2", color="blue")
plt.plot(all_preds, label="Predicted NO2", color="red", alpha=0.7)
plt.xlabel("Time Index")
plt.ylabel("NO2")
plt.title("NO2 Predictions vs Actuals - 15 days window")
plt.legend()
plt.savefig("no2_predictions_15_days.png", dpi=300, bbox_inches="tight")
...
def plot_loss_history(loss_history):
    ...
    plt.tight_layout()
    plt.savefig("loss_curve_no2_15_days.png", dpi=300)
